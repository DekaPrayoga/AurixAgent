...
export class ContextManager {
  ...
  pruneToolResults(messages: Message[]): Message[] {
    const lastToolResults = new Map<string, number>();
    const toPrune: number[] = [];

    for (let i = messages.length - 1; i >= 0; i--) {
      const msg = messages[i];
      if (msg.role === 'tool' && msg.toolCallId) {
        if (lastToolResults.has(msg.toolCallId)) {
          toPrune.push(i);
        } else {
          lastToolResults.set(msg.toolCallId, i);
        }
      }
    }
    ...
    return messages.map((msg, i) => {
      if (toPrune.includes(i)) {
        return { ...msg, content: `[pruned: old tool result]` };
      }
      return msg;
    });
  }
  ...
  private async summarize(text: string): Promise<string> {
    try {
      const res = await this.provider.chat([
        {
          role: 'system',
          content: 'Summarize this conversation concisely. Keep: key decisions, important facts, file paths, code snippets, and any unresolved questions. Format as bullet points. Be brief — this summary replaces the full conversation.',
        },
