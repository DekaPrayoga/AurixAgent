...
function getDefaultRepo(): string {
  ...
  try {
    const remote = execSync('git remote get-url origin 2>/dev/null', { encoding: 'utf8', timeout: 5000 }).trim();
...
- stats: contributors, activity (repo analytics)`,
  parameters: {
    type: 'object',
    properties: {
      action: { type: 'string', description: 'Action group: branch, pr-review, release, merge-conflict, actions, stats' },
      sub_action: { type: 'string', description: 'Sub-action — depends on action. branch: create|switch|list|delete|merge. pr-review: approve|changes|comment. release: create|list. merge-conflict: detect|show|abort. actions: list|status. stats: contributors|activity.' },
      ...
      repo: { type: 'string', description: 'Repository as owner/repo (default: connected or git remote)' },
  ...
  async execute(args: Record<string, unknown>): Promise<string> {
      ...
      try {
        switch (sub) {
          case 'create': {
            ...
            if (!name) return 'Usage: git_advanced action=branch sub_action=create name=<branch> [base=main]';
    ...
    if (action === 'pr-review') {
      ...
      if (!prNum) return 'Usage: git_advanced action=pr-review sub_action=<approve|changes|comment> pr_number=<n>';

      try {
        const body = (args.review_body as string) || '';
        switch (sub) {
          case 'approve':
            await ghApi(`/repos/${repo}/pulls/${prNum}/reviews`, 'POST', { event: 'APPROVE', body: body || 'LGTM! Approved via Aurix Agent.' });
            return `PR #${prNum} approved.`;
          case 'changes':
            await ghApi(`/repos/${repo}/pulls/${prNum}/reviews`, 'POST', { event: 'REQUEST_CHANGES', body: body || 'Requesting changes.' });
            return `PR #${prNum}: changes requested.`;
          case 'comment':
            if (!body) return 'Provide review_body.';
            await ghApi(`/repos/${repo}/pulls/${prNum}/reviews`, 'POST', { event: 'COMMENT', body });
            return `Review comment posted on PR #${prNum}.`;
          default:
            return `pr-review sub-actions: approve, changes, comment`;
        }
      } catch (e: any) { return `GitHub API error: ${e.message}`; }
    }
      ...
      try {
        switch (sub) {
          case 'create': {
            ...
            if (!tag) return 'Usage: git_advanced action=release sub_action=create tag=v1.0.0 [release_name=...] [release_body=...]';
            const name = (args.release_name as string) || tag;
            try { runGit(`tag ${tag} -m "${name}"`); runGit(`push origin ${tag}`); } catch {}
            const release = await ghApi(`/repos/${repo}/releases`, 'POST', {
              tag_name: tag, name, body: (args.release_body as string) || '',
              draft: args.draft as boolean || false, prerelease: args.prerelease as boolean || false,
            });
          ...
          default: return `release sub-actions: create, list`;
        }
      } catch (e: any) { return `Release error: ${e.message}`; }
    }
    ...
    if (action === 'merge-conflict') {
      try {
        switch (sub) {
          ...
          default: return `merge-conflict sub-actions: detect, show, abort`;
        }
      } catch (e: any) { return `Merge conflict error: ${e.message}`; }
    }
      ...
      try {
        switch (sub) {
          case 'list': {
            ...
            return runs.workflow_runs.map((r: any) => {
          ...
          default: return `actions sub-actions: list, status`;
        }
      } catch (e: any) { return `Actions error: ${e.message}`; }
    }
        ...
        switch (sub) {
          case 'contributors': {
            ...
            return `Top contributors — ${repo}:\n${contribs.map((c: any, i: number) =>
              `  ${i + 1}. ${c.login} — ${c.contributions} commits`).join('\n')}`;
          }
          case 'activity': {
          ...
          default: return `stats sub-actions: contributors, activity`;
        }
      } catch (e: any) { return `Stats error: ${e.message}`; }
    }

    return `Unknown action: "${action}". Available: branch, pr-review, release, merge-conflict, actions, stats\n` +
